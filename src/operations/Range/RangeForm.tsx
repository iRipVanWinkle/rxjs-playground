...
const formSchema = z.object({
  ...
  count: z.union([
    z.literal(''),
    z.coerce.number().nonnegative("The count must be positive value or empty."),
  ]).transform((val): number | undefined => val === '' ? undefined : val).optional()
});

export default function RangeForm() {
  const { getNode, updateNodeData } = useReactFlow<RangeNodeType>();
    ...
    <form onSubmit={form.handleSubmit(onSubmit)} className="grid gap-3">
      <FormField control={form.control} name="start" render={({ field }) => (
        <FormItem className="grid grid-cols-3 items-center gap-x-4 gap-y-1">
          <FormLabel className="col-span-1">Start</FormLabel>
